...
Scenario: where console login events across accounts are consolidated to one event bus
    Given an existing SPA system has been deployed on account '111111111111'
...
Scenario: where console logins events are processed as part of shadow updates
    Given an existing SPA system has been deployed on account '111111111111'
...
Scenario: where console logins appear in the monitoring dashboard
